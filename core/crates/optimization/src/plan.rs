...
/// tokens) carry no action and are dropped by [`build_plan`], so they cannot appear in a plan.
...
///
/// 1. **Classify & drop empties.** `Some(pool)` becomes [`StepKind::Swap`]; a no-pool cell with
///    `token_in == token_out` becomes [`StepKind::Carry`]; a no-pool cell with distinct tokens is
///    layout padding and is dropped.
...
mod tests {
    ...
    fn pruning_barely_changes_value() {
        ...
        let plan = build_plan(&flows, tokens::USDC.address, input, 1e-4);
    ...
    #[test]
    fn plan_has_only_swap_or_carry_and_groups_sum_to_one() {
        // Every step is an executable action (no empty cells survive), and each token's surviving
        // shares within a stage sum to one — nothing is stranded, nothing over-committed.
        let reserves = deep_no_arbitrage_universe();
        let model = init_model(reserves.clone());
        let input = 1_000.0;
    ...
    fn pruning_drops_tiny_and_renormalizes_survivors() {
        ...
        let plan = build_plan(&flows, usdc, 1_000.0, 0.05);
    ...
    fn merge_folds_same_pool_direction_duplicates() {
        ...
        let plan = build_plan(&flows, usdc, 100.0, 1e-4);
    ...
    proptest! {
        ...
        fn build_then_replay_stays_finite_and_conservative(
            ...
            let flows = model.extract_flows(input).expect("extract_flows failed");
            let plan = build_plan(&flows, usdc, input, 1e-4);
            let replayed = replay_plan(&plan, &reserves).expect("replay_plan failed");
